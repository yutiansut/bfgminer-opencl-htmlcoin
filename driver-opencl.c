...
bool _set_kernel(struct cgpu_info * const cgpu, const char *_val)
{
	...
	int dummy_srclen;
	enum cl_kernels hinterface;
	struct mining_algorithm *malgo;
	char *src = opencl_kernel_source(filename, &dummy_srclen, &hinterface, &malgo);
	if (!src)
		return false;
	free(src);
	if (!malgo)
...
const struct opencl_kernel_info *opencl_scanhash_get_kernel(struct cgpu_info * const cgpu, _clState * const clState, const struct mining_algorithm * const malgo)
{
	...
	if (!kernelinfo->loaded)
	{
		...
		kernelinfo->queue_kernel_parameters = kernel_interfaces[kernelinfo->hinterface].queue_kernel_parameters_func;
	}
	return kernelinfo;
}
...
static int64_t opencl_scanhash(struct thr_info *thr, struct work *work, int64_t __maybe_unused max_nonce)
{
	...
	if (thrdata->res[found]) {
		...
		postcalc_hash_async(thr, work, thrdata->res, kinfo->hinterface);
